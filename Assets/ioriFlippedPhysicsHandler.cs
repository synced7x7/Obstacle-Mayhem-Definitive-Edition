...
public class ioriFlippedPhysicsHandler : MonoBehaviour
{
    ...
    public void xy_physics()
    {
        ...
        {
            body.linearVelocity = new Vector2(-1 * Mathf.Max(20f - deceleration, 0f), body.linearVelocity.y);
                ...
                else if (data.keyDownPressCount > 0 && !data.movementFlag) // Down Arrow is pressed first
                {
                    ...
                    else
                    {
                        if (!data.devotion_to_the_inferno)
                        {
                            ...
                            if (data.keyLeftPressCount == 0 && data.keyAPressCount > 0 && data.keyRightPressCount == 1 && !anim.GetBool("heavy_punch1"))
                            {
                                S_A.basic_combo(8);
                            }
                            // Execute second move (Down + Left + A, repeated)
                            else if (data.keyLeftPressCount == 0 && data.keyRightPressCount == 2 && data.keyAPressCount > 0 && anim.GetBool("heavy_punch1") && !anim.GetBool("heavy_punch2") && Kyo_data.Instance.bloom_of_desolation_hit > 0)
                            {
                                S_A.basic_combo(9);
                            }
                            // Execute third move (Down + Left + A, repeated again)
                            else if (data.keyLeftPressCount == 0 && data.keyRightPressCount == 3 && data.keyAPressCount > 0 && !anim.GetBool("heavy_punch1") && anim.GetBool("heavy_punch2") && !anim.GetBool("heavy_punch3") && Kyo_data.Instance.bloom_of_desolation_hit > 0)
                            {
                                S_A.basic_combo(10);
                            }
                            #endregion
                            ...
                            else if (data.keyRightPressCount == 2 && data.keyDownPressCount == 2 && data.keySPressCount > 0 && !anim.GetBool("iori_run6"))
                            {
                            ...
                            else if (data.keyDownPressCount == 1 && data.keyLeftPressCount == 1 && data.keyAPressCount > 0)
                            {
                                S_A.basic_combo(12);
                                return;
                            }
                            //around the world
                            else if (data.keyDownPressCount == 2 && data.keyLeftPressCount == 2 && data.keyAPressCount > 0)
                            {
                                S_A.basic_combo(13);
                                return;
                            }
                        ...
                        //infernal blood lust
                        else if (data.keyRightPressCount == 2 && data.keyDownPressCount == 2 && Input.GetKeyDown(KeyCode.Q))
                        {
                            speedyanimationBGBehaviour.Instance.speedyAnimationActivator();
                ...
                if (!data.movementFlag)
                {
                    ...
                    if (!anim.GetBool("iori_sit7") && data.isbackjumping == false && !data.isdodging)
                    {
                        ...
                        if (data.rightKeyPressCount < 2 && (anim.GetBool("Iori_right_mov2") || anim.GetBool("Iori_left3"))) //LR movement controlled by input 
                        {
                            ...
                            {
                                if (anim.GetBool("Iori_right_mov2")) //rightmove velo
                    ...
                    else if (!data.isdodging)
                    {
                        ...
                        if (Input.GetKeyDown(KeyCode.RightArrow) && !data.isbackjumping)
                            HandleRightKeyPress();
                        ...
                        {
                            if (data.rightKeyPressCount >= 2 ) //BackJump
                            {
                            ...
                            {
                                S_A.iori_sprite_activator(3); // Walking right
                            }
                        }
                        else if (horizontal_input < 0) // Moving left
                        {
                            if (data.leftKeyPressCount == 2) //Run
                            {
                                S_A.iori_sprite_activator(5);
                            }
                            else
                            {
                                S_A.iori_sprite_activator(2); // Walking left
                            }
                        }
    ...
    private void HandleLeftKeyPress()
    {
        if (Time.time - data.lastKeyLeftPressTime_independent < 0.2f && data.grounded) // Detect double press within 0.2 seconds
        {
            ...
            //Debug.Log("leftkey incremented to "+ data.leftKeyPressCount);
        }
        else
        {
