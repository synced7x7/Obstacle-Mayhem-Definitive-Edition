...
public class ionfernalBloodLustSPBehaviour : MonoBehaviour
{
    public static ionfernalBloodLustSPBehaviour Instance { get; private set; }
    ...
    [SerializeField] private float timeBetweenFade = 0.25f;
    ...
    public void StartBackgroundMove(float startPosition, float endPosition)
    {
        ...
        transform.position = new Vector3(startPosition, transform.position.y, 0.1f); // Ensure correct initial z position
...
//            Debug.Log("Tranform position " + transform.position.z + " iori Position: " + ioriTransform.position.z);
            ...
            transform.position = Vector3.MoveTowards(transform.position,
                new Vector3(endPosition, transform.position.y, 0.1f),
                currentSpeed * Time.deltaTime);

            yield return null;
        }

        transform.position = new Vector3(endPosition, transform.position.y, 0.1f);
        isMoving = false;
